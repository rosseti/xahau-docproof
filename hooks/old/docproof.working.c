...
//  const timestamp = Math.floor(Date.now() / 1000); // Timestamp Unix atual (em segundos)
// const HookParameter = {
//     HookParameterName: Buffer.from("Timestamp").toString("hex"),
//     HookParameterValue: timestamp.toString(16).padStart(8, "0"),
// };
...
int64_t hook(int32_t reserved)
{
    ...
    uint8_t doc_id[32];
    uint64_t doc_id_size = otxn_param(SBUF(doc_id), "DocId", 5);
    ...
    sig.timestamp = ledger_last_time();
