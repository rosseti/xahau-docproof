...
int64_t hook(int32_t reserved)
{
    ...
    int64_t doc_id_size = otxn_param(SBUF(sig.doc_id), "DocId", 5);
    ...
    Signature sig_exists;
    int64_t existing = state(SVAR(sig_exists), SBUF(state_key_hex));
    if (existing > 0)
    {
        TRACEHEX(sig_exists.doc_hash);
        TRACEHEX(sig_exists.signer_acc);
        TRACEHEX(sig_exists.doc_id);
